...
>&2 echo "INFO - GIT tag-commit: $GIT_DESCRIBE ; RPM version-release: $TARGET_VERREL"

# archive tarball from Git workspace
export TARGET_TARBALL_NAME=${NAME}-$TARGET_VERSION
...
TARGET_SPEC_PATH=$TARGET_SPEC_DIR/${NAME}.spec

sed -e '1 i\%global _enable_debug_package 0\n%global debug_package %{nil}' \
    -e "s/^\(Version:\).*$/\1        ${TARGET_VERSION}/" \
    -e "s/^\(Release:\).*$/\1        ${TARGET_RELEASE}%{?dist}/" \
    -e "s/^\(Source0:\).*$/\1        ${TARGET_TARBALL_NAME}.tar.gz/" \
    -e 's/ asciidoc xmlto//g' \
    -e '/^%configure/ s/$/ --disable-documentation/g' \
    -r -e '/_docdir|^%doc/d' \
    "${TARGET_SPEC_PATH}".in > "${TARGET_SPEC_PATH}"

# build rpms
rpmbuild -bs --clean "$TARGET_SPEC_PATH" \
         --define "%_topdir $HERE" \
